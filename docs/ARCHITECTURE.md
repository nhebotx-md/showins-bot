...
Sumber chat ditentukan dari JID tujuan: `@s.whatsapp.net`/`@lid` sebagai private, `@g.us` sebagai group, dan `@newsletter` sebagai channel. Tabel aktivitas memakai separator tetap dengan kolom `TIME`, `FLOW`, `CONTEXT / DELIVERY`, dan `ACTION`. Command dicetak sebagai `USER → BOT` beserta sumber serta role; setelah `sendMessage` berhasil, logger mencetak `BOT → USER` beserta `DELIVERED → target`. Target chat pribadi disamarkan pada terminal. Pengaturan lingkungan `NO_COLOR=1` menonaktifkan warna ANSI tanpa mengubah struktur data log.
